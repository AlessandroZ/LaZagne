...
class Chrome(ModuleInfo):
	...
	def chrome_decrypt(self, encrypted, iv, key): #AES decryption using the PBKDF2 key and 16x ' ' IV, via openSSL (installed on OSX natively)
		...
		try: 
			decrypted = subprocess.check_output("openssl enc -base64 -d -aes-128-cbc -iv '{iv}' -K {hex_key} <<< {hex_enc_password} 2>/dev/null".format(iv=iv, hex_key=hex_key, hex_enc_password=hex_enc_password), shell=True)
	...
	def run(self, software_name=None):
		...
		if not self.safe_storage_key:
			...
			print_debug('INFO', 'Chrome safe storage key has been retrieved: {safe_storage_key}'.format(safe_storage_key=self.safe_storage_key))
