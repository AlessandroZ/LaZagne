...
class LsaDecryptor():
	...
	def acquire_crypto_material(self):
		...
		if not sigpos:
			...
			return
		print_debug('DEBUG', 'Lsa signature {signature} found at offset {offset}'.format(signature=self.SIGNATURE.encode('hex'), offset=sigpos))
