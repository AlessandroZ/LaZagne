...
class IISCentralCertP(ModuleInfo):
    ...
    def create_RSAKeyValueFile(self, exe_file, container):
        ...
        try:
            ...
            print_debug('DEBUG', u'Error executing {container}'.format(container=container))
            tmp_file = ''
    ...
    def run(self):
        ...
        ccp_enabled = self.get_registry_key('HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\IIS\CentralCertProvider' ,'Enabled')
        if (ccp_enabled != 1 ):
            print_debug('DEBUG', u'IIS CentralCertProvider is not enabled')
            return
        ...
        if (rsa_xml_file == ''):
            print_debug('DEBUG', u'Problems extracting RSA Key Value')
            return
        ...
        CertStoreLocation = self.get_registry_key('HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\IIS\CentralCertProvider' ,'CertStoreLocation')
        values['CertStoreLocation'] = CertStoreLocation
        
        username = self.get_registry_key('HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\IIS\CentralCertProvider' ,'Username')
