...
import struct
import sys
from ctypes import (POINTER, byref, c_int, c_long, c_size_t, c_ssize_t,
                    c_void_p, create_string_buffer, get_errno)
...
class LinProcess(BaseProcess):
    ...
    def check_ptrace_scope(self):
        ...
        try:
            ...
            if os.getuid() == 0:
                ...
                logger.warning(
                    "yama/ptrace_scope == 1 (restricted). you can't ptrace other process ... get root")
            elif ptrace_scope == 2:
                logger.warning(
                    "yama/ptrace_scope == 2 (admin-only). Warning: check you have CAP_SYS_PTRACE")
    ...
    def iter_region(self, start_offset=None, end_offset=None, protec=None, optimizations=None):
        ...
        with open("/proc/" + str(self.pid) + "/maps", 'r') as maps_file:
            for line in maps_file:
                m = re.match(
                    r'([0-9A-Fa-f]+)-([0-9A-Fa-f]+)\s+([-rwpsx]+)\s+([0-9A-Fa-f]+)\s+([0-9A-Fa-f]+:[0-9A-Fa-f]+)\s+([0-9]+)\s*(.*)', line)
                if not m:
                    continue
                start, end, region_protec, offset, dev, inode, pathname = int(m.group(1), 16), int(
                    m.group(2), 16), m.group(3), m.group(4), m.group(5), int(m.group(6)), m.group(7)
    ...
    def write_bytes(self, address, data):
        ...
        for i in range(0, len(data), word_size):
            ...
            # we need to let some data untouched, so let's read at given offset to complete our 8 bytes
            if len(word) < word_size:
                existing_data = self.read_bytes(
                    int(address) + i + len(word), bytes=(word_size - len(word)))
                word += existing_data
            if sys.byteorder == "little":
                word = word[::-1]

            attempt = 0
            err = c_ptrace(ctypes.c_int(PTRACE_POKEDATA), c_pid,
                           int(address) + i, int(word.encode("hex"), 16))
            if err != 0:
    ...
    def read_bytes(self, address, bytes=4):
        ...
        if not LARGE_FILE_SUPPORT:
            ...
            try:
                ...
                logger.info("Error reading %s at %s: %s" %
                            ((bytes), address, e))
        if self.read_ptrace:
            self.ptrace_detach()
        return data
